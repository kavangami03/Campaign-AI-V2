...
export function Hero() {
  ...
  useEffect(() => {
    ...
    /* Failsafe. The headline is the most important text on the page, so it
       ...
       Comfortably longer than the intro (~2.35s) so it only ever fires when
    ...
    const failsafe = window.setTimeout(() => setState("revealed"), 4000);
    ...
    const ctx = gsap.context(() => {
      ...
      playBriefSequence(brief).delay(1);
